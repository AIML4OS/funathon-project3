...
4/cf04bf4c53e84f6baf1d15f928100fb2/artifacts/
```

Two artifacts are needed:

- **`model/`** — the full MLflow model directory (weights, code, environment)
- **`params.json`** — a JSON file containing the training hyperparameters
...
4. Fetch `params.json` over HTTPS with `requests` and parse the hyperparameters

::: {.callout-caution}
## Exercise 1 — Load a pre-trained model from public S3
...
4. Fetch `params.json` over HTTPS and parse `n_bands`, `tiles_size`,
   `augment_size`, `module_name`, `normalization_mean`, `normalization_std`
:::
...
params_url = __  # TODO: public HTTPS URL to params.json
...
```{python}
#| eval: true
# This block runs silently to set up shared variables for exercises 6, 7 and 8.
import numpy as np
import geopandas as gpd
import folium
from folium.raster_layers import ImageOverlay
from rasterio.warp import transform_bounds
from intermediate_solutions.solution_step3.utils import get_satellite_image
```
...
1. Choose a GPS point in Luxembourg (e.g. the Eurostat offices)
...
## Exercise 7 — Predict a single image via the API
...
## Exercise 8 — Predict an entire NUTS3 region
...
- `response_nuts.json()` returns a GeoJSON string; `json.loads()` parses it
  into a dict before calling `from_features()`.
