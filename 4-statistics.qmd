...
The API returns a GeoJSON FeatureCollection of polygons. Each polygon carries
a `label` attribute (integer from 1 to 10) corresponding to one of the 10
CLC+ Backbone land-cover classes. We load this directly into a GeoDataFrame
and enrich it with area measurements and human-readable class names.
...
5. Print the result sorted by `total_area_km2` descending
:::
...
groups and print a concise summary with area and share.
...
3. Compute the share as a percentage of `total_km2`
4. Print a formatted summary line for each group
:::
...
**Goal:** Apply the same aggregation as Exercise 1, but this time on
`gdf_nuts` which covers the entire NUTS3 region. Then compare the class
shares between the single tile and the full region.
...
3. Print the result
4. Build a side-by-side comparison of tile vs. NUTS3 shares using
   `pd.concat()`
:::
...
comparison = pd.concat([__, __], axis=1).fillna(0)  # TODO: tile_shares, nuts_shares
print(comparison.sort_values("__", ascending=False).to_string())  # TODO: sort column
```
...
- Rename the share columns `"tile_share_pct"` and `"nuts3_share_pct"` for
  clarity.
- Sort by `"nuts3_share_pct"` to see dominant classes at the NUTS3 level.
:::
...
::: {.callout-caution}
## Exercise 6 — Build an interactive Folium map of the NUTS3 predictions

**Goal:** Display all predicted polygons for the NUTS3 region on an
interactive Folium map. Sealed polygons (label 1) should stand out with
higher opacity so that built-up zones are immediately visible. Hovering
over a polygon should show its class name and area.

**Steps:**

1. Reproject `gdf_nuts` to EPSG:4326
2. Compute the centroid to centre the map
3. Create a `folium.Map` at zoom level 10
4. Add a `GeoJson` layer with a `style_function` that colours each polygon
   using `label_to_color` and sets `fillOpacity` to `0.7` for sealed
   (label 1) and `0.4` otherwise
5. Add a `GeoJsonTooltip` showing `class_name` and `area_km2`
:::
...
folium.GeoJson(
    __,  # TODO: reprojected GeoDataFrame
    style_function=lambda feature: {
        "fillColor":   label_to_color.get(feature["properties"]["__"], "#808080"),  # TODO: property key
        "color":       "black",
        "weight":      0.3,
        "fillOpacity": __ if feature["properties"]["label"] == 1 else __,  # TODO: opacities
    },
    tooltip=folium.GeoJsonTooltip(
        fields=["__", "__"],          # TODO: fields to display
...
- Use `zoom_start=10` for a NUTS3-level view.
- The property key for the colour lookup is `"label"`.
- Set sealed opacity to `0.7` and all others to `0.4`.
- Tooltip fields: `["class_name", "area_km2"]`.
:::
...
statistics, and compare shares with the 2024 results using a grouped
...
4. Build `tile_comparison` with columns `"2021 (%)"`, `"2024 (%)"`,
   `"change (pp)"`
5. Plot a grouped horizontal bar chart with both years side by side
:::
...
2021 predictions from `/predict_nuts`, compute statistics, and produce a
printed change table, an absolute area summary, and a two-panel figure.
...
5. Print the absolute area change for Sealed, Forest, Agricultural, Water
6. Plot the two-panel figure (grouped bars + change bars coloured green/red)
:::
